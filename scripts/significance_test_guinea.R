...
# Load cleaned data
data <- readRDS(here::here("data", "cleaned_data_guinea.rds"))
...
Hyp1_Table <- hyp1_results %>%
  ...
  gt() %>%
  tab_header(
    title = "Guinea-Bissau — Hypothesis 1: Association Between Age Group and Type of Registrant",
...
gtsave(Hyp1_Table, filename = here::here("outputs", "Hypothesis_1_Table_gb.png"))
...
Hyp2_Table <- results_df %>%
  gt() %>%
  tab_header(
    title = "Guinea-Bissau — Chi-squared Test: Relationship Type by Gender (within Age Group)",
...
gtsave(Hyp2_Table, filename = here::here("outputs", "Hypothesis_2_Table_gb.png"))
