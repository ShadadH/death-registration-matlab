...
chisq.test(table_male)  # Chi-squared test for males

# Remove columns and rows with all zeros
table_male_clean <- table_male[rowSums(table_male) > 0, colSums(table_male) > 0]
chisq.test(table_male_clean)

# Repeat for females
table_female <- table(female_data$age_group_broad, female_data$relation_group)
table_female_clean <- table_female[rowSums(table_female) > 0, colSums(table_female) > 0]
chisq.test(table_female_clean)

hyp1_results <- data.frame(
  Gender = c("Male", "Female"),
  Chi_Squared = c(33.878, 21.524),
  df = c(12, 12),
  p_value = c(0.000705, 0.04321),
