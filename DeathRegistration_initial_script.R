...
# Check if "Unknown" is completely gone
table(data_filtered$relation_group, useNA = "always")
table(data$relation_group, data$age_group_broad, data$d_sex
      ,useNA = "always")



# Mosaic plot with proper labeling
ggplot(data_filtered) +
  ...
  labs(
    ...
    y = "Proportion",  # Removes y-axis label
  ...
  theme(
    ...
    axis.text.x = element_text(angle = 90, hjust = 1, size = 8)  # Rotates x-axis labels for readability
  ) +
  facet_wrap(~ d_sex)
